...
public class ContratSalade implements Contrat {
	...
	public void comptePoints(ArrayList<Joueur> joueurs) {
		...
		for (int i = 0; i < joueurs.size(); i++) {
			...
			Joueur joueur = joueurs.get(i); // On fait joueur par joueur, et on simplifie pour l'utilisation du joueur i
											// plus tard
			while (!joueur.getPlis().isEmpty()) { // Tant que le joueur a des plis
				...
				if (c.getValeur() == 13 && c.getSymbole().equals("Coeur")) { // On donne les points du roi s'il y est
					cptroi++;
				}
				joueur.getPlis().remove(c); // On enl̬ve une carte
			}
			joueur.getPoints().add(plis + cptcoeur * 10 + cptdame * 20 + cptroi * 80);
		}
