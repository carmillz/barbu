...
public class Jeu {
    ...
    public void tourReussite(int debut){
        for (int i = 0; i<joueurs.size(); i++){
            for (int j = 0; j<4; j++){
                System.out.println(((ArrayList<ArrayList<String>>)plateau).get(j));
            }
    ...
    public void ajoutReussite(Carte carte){
        ...
        int posI=positionSymbole(carte.getSymbole());
        int posJ=carte.getValeur()-2;
        ((ArrayList<ArrayList<String >>)plateau).get(posI).set(posJ,carte.toString());
    }

    public int positionSymbole(String symbole){
        if (symbole.equals("Coeur")){
            return 0;
        }else if (symbole.equals("Trèfle")){
            return 1;
        }else if (symbole.equals("Carreau")){
            return 2;
        }else {
            return 3;
        }
